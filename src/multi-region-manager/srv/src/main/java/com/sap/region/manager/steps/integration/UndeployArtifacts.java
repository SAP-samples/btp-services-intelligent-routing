...
/**
 * <p>
 * UndeployArtifacts class.
 * </p>
 *
 * @author shanthakumar.krishnaswamy@sap.com
 */
public class UndeployArtifacts implements Tasklet {
    private static final Logger logger = LoggerFactory.getLogger(DeployArtifacts.class);
    private Properties properties;
    private String[] packageIds;
    private String region;

    public UndeployArtifacts(Properties properties, String[] packageIds, String region) {
        ...
        for (String packageId : packageIds) {
            ...
            try {
                ...
                for (String designtimeArtifactId : designtimeArtifactIds) {
                    logger.info("Undeployment started for artifact Id: " + designtimeArtifactId);
                    if (!Helper.isArtifactDeployed(designtimeArtifactId, properties, client)) {
                        logger.info("Artifact not deployed: " + designtimeArtifactId);
                        continue;
                    }
                    HttpRequest request = HttpRequest.newBuilder()
                            .uri(new URI(properties.getProperty("url") + "/api/v1/IntegrationRuntimeArtifacts('"
                                    + designtimeArtifactId + "')"))
                    ...
                    if (response.statusCode() == 202) {
                        logger.info("Undeployment successfully submitted for artifact Id: " + designtimeArtifactId);
                    } else {
                        logger.error(response.body());
                        throw new Exception("Error in undeploying artifacts. Region: [" + region + "] Response code: ["
                ...
                contribution.setExitStatus(new ExitStatus(contribution.getExitStatus().getExitCode(),
                        "Undeployment successfully completed. Region: [" + region + "] Artifact Ids: "
